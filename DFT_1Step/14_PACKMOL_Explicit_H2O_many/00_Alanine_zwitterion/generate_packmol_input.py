...
# ===================================================================================
# 3. Packmol base input 파일 생성 (15×15×15 Å 박스 사용)
# ===================================================================================

box_size = 15.0                # 박스 한 변의 길이 [Å]
half_box = box_size / 2.0      # 중심(0,0,0) 기준 절반 길이

with open(output_path, 'w') as f:
    f.write("tolerance 2.0\n")
    f.write("filetype xyz\n")
    f.write("output case_{index}.xyz\n\n")
    
    # 1) 중심 분자 고정 배치 (가운데에 정렬된 rotated_xyz 사용)
    f.write(f"structure ../{rotated_xyz_path}\n")
    f.write("  number 1\n")
    f.write("  fixed 0. 0. 0. 0. 0. 0.\n")
    f.write("end structure\n\n")
    
    # 2) Solvent 분자들은 15×15×15 Å 박스 안에 랜덤 배치
    for solvent in solvents:
        count = solvent['count']
        f.write(f"structure ../{solvent['file']}\n")
        f.write(f"  number {count}\n")
        f.write(f"  inside box {-half_box:.2f} {-half_box:.2f} {-half_box:.2f} "
                f"{half_box:.2f} {half_box:.2f} {half_box:.2f}\n")
        f.write("end structure\n\n")

    # 3) Seed 및 초기 무작위 위치 설정
    f.write("seed {seed}\n")
